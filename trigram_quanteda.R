...
df.3 <- data.frame(word=names(freqTerms.3), freqTerms.3)
...
predictText<-function(n) predict(fit.3$estimate, newdata=as.vector(unlist(strsplit(n," ", fixed=TRUE))), n.ahead = 8)
predictText(inWord)
