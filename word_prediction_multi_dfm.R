...
# number of max rcds to be processed at one time
dfm.max.rcds <- 50000

sample.len <- length(sampleText)
loopTimes <- as.integer(sample.len/dfm.max.rcds)

# initial dfm file
dfm.3 <- dfm(sampleText[1:dfm.max.rcds], ngrams=3,
...
# loop and build large dfm file
for (idx in 2:loopTimes) {
  # get start/end range of sample index
  start <- ((idx-1)*dfm.max.rcds)+1
  end <- idx*dfm.max.rcds
  cat(start, ":", end, "\n")
  
  dfm.3.temp <- dfm(sampleText[start:end], ngrams=3,
               toLower = TRUE,concatenator = " ", removeNumbers = TRUE, removeSymbols=TRUE,
               removePunct = TRUE, removeSeparators = TRUE, removeTwitter = TRUE, removeURL=TRUE)

  dfm.3 <- rbind(dfm.3, dfm.3.temp)
  rm(dfm.3.temp)
  gc()
}

# Get last records in sample file
end <- idx*dfm.max.rcds
dfm.3.temp <- dfm(sampleText[end:sample.len], ngrams=3,
               toLower = TRUE,concatenator = " ", removeNumbers = TRUE, removeSymbols=TRUE,
               removePunct = TRUE, removeSeparators = TRUE, removeTwitter = TRUE, removeURL=TRUE)
